<div class="container">
    <script>
        ...
        function confirmDelete( obj ) {
            ...
            if( confirm('Are you sure you want to delete this item?') ) {
               window.location = '<?php echo BASE_URL.'browse-alert.php?del=' ?>' + $(obj).attr('data-id');
            }
        }
    ...
    if( isset( $_GET['del'] ) ) {
        ...
        if( $db->queryDeleteDb( $deleteQuery ) ) {
            echo '<div class="alert alert-success" role="alert">Alert has been <strong>successfully</strong> deleted</div>';
        } else {
...
<?php
    ...
    $sqlQuery = "SELECT `message_id`, `datetime`, `description`, `sent` FROM `message` WHERE `update_type` = '0' ORDER BY `sent`, `datetime`";
    ...
    if(  $query =  $db->queryDb( $sqlQuery ) ) {
        ...
        foreach ($query as $value) {
            ...
            if( $value['sent'] ) {
                ...
                $button = '<a href="edit-alert.php?id='.$value['message_id'].'" class="btn btn-xs btn-warning">Edit</a> <a href="javascript:void(0);" data-id="'.$value['message_id'].'" class="btn btn-xs btn-danger" onclick="confirmDelete(this)">Delete</a>';
            }
